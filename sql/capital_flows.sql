...
--
-- =============================================================
-- Pre-aggregated capital flows for a period of market volatility.
--
-- Production query for the capital flows cityscape visualization.
--
-- Session-based capital flow model with proportional allocation.
--
-- How it works:
--   Trades are grouped into sessions (5-min gap within same day).
--   Within each session, sells and buys are aggregated per building,
--   then joined to create building-to-building flow pairs.
--   Flow amounts use proportional allocation: each sell-building's
--   contribution to a buy-building is weighted by that buy-building's
--   share of the session's total buy amount.
--
--   Results are grouped by source/destination at all three levels
--   of the cityscape hierarchy (district/neighborhood/building).
--
-- HAVING COUNT(*) >= 10 filters noise (low-frequency pairs).
--
-- Covers Feb 20 - Mar 13, 2026.
-- =============================================================

WITH

-- ---------------------------------------------------------------
-- Lookup CTEs (identical to v3)
-- ---------------------------------------------------------------
...
-- Single account_master scan: avoids scanning this table 3 times
account_master AS (
...
order_level AS (
  ...
  WHERE ft.trade_date_key BETWEEN 20260220 AND 20260313
...
classic_trades AS (
  ...
  WHERE td.process_date BETWEEN '2026-02-20' AND '2026-03-13'
...
-- ---------------------------------------------------------------
-- Source 3: Classic options from trades
-- ---------------------------------------------------------------
...
-- ---------------------------------------------------------------
-- Source 4: Classic futures from trades
-- ---------------------------------------------------------------
...
bond_mf_activities AS (
  ...
  WHERE a.type = 'TRADE'
    ...
    AND a.activity_date BETWEEN '2026-02-20' AND '2026-03-13'
),

-- ---------------------------------------------------------------
-- Source 5 continued: Bonds and mutual funds
-- ---------------------------------------------------------------
...
-- This avoids the N×M per-trade cross-product — a session with
-- 500 sells across 10 tickers and 500 buys across 8 tickers
-- produces 80 rows instead of 250,000.
-- ---------------------------------------------------------------
...
-- Pre-filter: only buildings appearing in >= 10 distinct sessions as sells
-- A building in fewer than 10 sell-sessions can never produce >= 10 pairs
-- with any single buy-building, so it would be eliminated by the final HAVING.
...
-- Total buy amount per session (for proportional allocation)
...
-- ---------------------------------------------------------------
-- Join sell-buildings to buy-buildings within same session.
-- Proportional allocation: each sell-building's flow to a
-- buy-building is weighted by that buy-building's share of
-- the session's total buy amount.
-- ---------------------------------------------------------------
...
-- ---------------------------------------------------------------
-- Final aggregation: one row per directional pair per trade_date
-- Postprocessor collapses dates into pair-level aggregates + daily arrays.
-- Uses window function to filter qualifying pairs (>= 10 sessions) inline.
-- ---------------------------------------------------------------
